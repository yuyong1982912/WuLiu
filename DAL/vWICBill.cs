...
namespace Maticsoft.DAL
{
	...
	public partial class vwICBill
	{
	 ...
	 #region 连接到金蝶数据库
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select a.FInterID,a.FEntryID,a.Fdate,a.FbillNo,a.FSupplyID,a.FNumber,a.Fmodel,a.FSourceTranType,a.FSourceBillNo,Cast(a.Fauxqty - ISNULL(c.FQty,0) As Float) As Fauxqty,Cast(b.FSize As Float) As FSize,Cast(b.FGrossWeight As Float) As  FGrossWeight");
			strSql.Append(" FROM vwICBill_8 a Left Join t_ICItem b on a.FNumber=b.FNumber ");
            strSql.Append(" Left Join WuLiu..KuaidiDetail c On c.FInterID = a.FbillNo And c.FNumber = a.FNumber ");
			if(strWhere.Trim()!="")
			{
